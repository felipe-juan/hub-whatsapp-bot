...
const titles = {
  ...
  calculators: 'Calculadoras', groups: 'Grupos', analytics: 'Estatísticas anônimas', learning: 'Aprendizado assistido', management: 'Sistema e manutenção', settings: 'Configurações', logs: 'Registros'
};
...
async function render(){
  ...
  const fn={dashboard:renderDashboard,messages:renderMessages,diagnostics:renderDiagnostics,calculators:renderCalculators,groups:renderGroups,analytics:renderAnalytics,learning:renderLearningSuggestions,management:renderManagement,settings:renderSettings,logs:renderLogs}[state.view];
...
async function renderLearningSuggestions(){
  ...
  content.innerHTML=`
    <div class="toolbar"><div><h2>🧠 Sugestões pendentes</h2><p class="muted">O bot nunca aprende sozinho. Aprovar adiciona a frase exatamente como exemplo de gatilho ao card sugerido.</p></div>${badge(`${items.length} pendente(s)`,items.length?'warning':'active')}</div>
    ${items.length?`<div class="conflict-list">${items.map(item=>`<article class="conflict-item"><div class="toolbar"><div>${badge(item.chat_type==='group'?'Grupo':'Privado')}<h3>${esc(item.suggested_title||'Sem card sugerido')}</h3></div><small>${fmtDate(item.last_seen_at)}</small></div><blockquote class="message-snippet">${esc(item.message_excerpt)}</blockquote><p class="muted">Confiança: ${Math.round(Number(item.confidence||0)*100)}% · Ocorrências: ${Number(item.occurrences||1)}${item.reasons?.length?` · ${esc(item.reasons.join('; '))}`:''}</p><div class="actions"><button class="button small primary approve-learning" data-id="${Number(item.id)}" ${item.suggested_message_id?'':'disabled'}>✅ Aprovar</button><button class="button small danger reject-learning" data-id="${Number(item.id)}">✖ Rejeitar</button></div></article>`).join('')}</div>`:'<div class="card empty">Nenhuma sugestão pendente.</div>'}
    <div class="toolbar section-gap"><div><h2>🧪 Frases de regressão</h2><p class="muted">Coleção permanente executada antes das atualizações. Cadastre frases que devem responder ou que devem permanecer ignoradas.</p></div><div class="actions"><button class="button" id="run-regression">▶ Executar testes</button><button class="button primary" id="new-regression">＋ Nova frase</button></div></div>
  ...
  $$('.approve-learning').forEach(button=>button.onclick=async()=>{button.disabled=true;try{await api(`/api/learning-suggestions/${button.dataset.id}/approve`,{method:'POST',body:'{}'});toast('✅ Frase adicionada ao card sugerido.');await renderLearningSuggestions();}catch(error){button.disabled=false;toast(error.message,true);}});
